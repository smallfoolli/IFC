...
import Student_filter_versatile_explore_feature_cat as models
...
def train_teacher(model, device, train_loader, optimizer, epoch):
    ...
    print(' Train loss is: {:.4f}, Train accuracy: {}/{} ({:.2f}%)'.format(train_loss, correct, len(train_loader.dataset),
...
def teacher_main(args, train_loader, test_loader):
    ...
    #         para_others += [para]
    #     # print(name)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.LRW, weight_decay=1e-5)
    ...
    for epoch in range(1, args.Epoch + 1):
        ...
        # if epoch % 10 == 0:
            # LR_W = LR_W / 5
            # LR = LR / 5
            optimizer = torch.optim.Adam(model.parameters(), lr=LR*0.9**(epoch-1), weight_decay=1e-5)
...
# LRW = [1e-3, 5e-3, 1e-2, 5e-2, 0.1, 0.25, 0.5, 0.75]
...
df_result.to_csv(args.Result_save_path + 'Student9 results_LRW_Var_search1_K77 on data3 cross person featureCat.csv', index=False, sep=',')
#
# teacher_model, teacher_history = teacher_main(args, train_loader, test_loader)
