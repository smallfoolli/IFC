...
parser.add_argument('--Temperature', type=float, default=[100.0], help='The Temperature')
...
def test_teacher(model, device, test_loader, Num_Class):
    model.eval()
    test_loss = 0
    correct = 0
    pro_vector_val = torch.zeros(1, Num_Class)
    with torch.no_grad():
...
#加载教师模型，看其输出分布
model = torch.load(args.Model_save_path + '/Trained teacher model')
#model = torch.load(args.Model_save_path + '/Trained teacher model on datasetp2')
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = model.to(device)
v1, v2, data_Pro = test_teacher(model, device, test_loader, args.Num_Class)

#存储概率向量
#参考https://jingyan.baidu.com/article/9c69d48ff3123d13c9024e06.html##1
#col_name = ['Class0', 'Class1', 'Class2', 'Class3', 'Class4', 'Class5', 'Class6', 'Class7', 'Class8', 'Class9']
col_name = ['Class0', 'Class1', 'Class2', 'Class3', 'Class4', 'Class5', 'Class6', 'Class7']
res = pd.DataFrame(columns=col_name, data=data_Pro.numpy()[1:, :])
res.to_csv(args.Result_save_path + '/teacher_output_distribution_s115/output_teacher_distribution.csv', index=False, sep=',')

'''
#画分布图
x = [i for i in range(args.Num_Class)]
for j in range(1, len(data_Pro.numpy()[:, 1])):
    ...
    plt.xticks([i for i in range(args.Num_Class)])
    # plt.xlim(-0.5, 9.5)
    #plt.show()
    plt.savefig(args.Result_save_path + '/teacher_output_distribution_s115/output_teacher_distribution_sample' + str(j) +'.png')
    plt.close()
'''
'''
